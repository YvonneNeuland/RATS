...
#include <stdexcept>
//--------------------------------------------------------------------------------
#pragma region HELPER_UTILS
namespace compile_time_util {
	template<bool = true> class assert_class { public: class private_class_if_failed{}; };

	template<> class assert_class<false> { private: class private_class_if_failed{}; };
}
//--------------------------------------------------------------------------------
#define OFFSET_OF(TYPE, MEMBER)	((reinterpret_cast<size_t>(&reinterpret_cast<const volatile char &>(reinterpret_cast<TYPE *> (1)->MEMBER) ) ) - 1)
#define COMPILE_TIME_ASSERT(expr) class CompileError : public compile_time_util::assert_class<(bool)(expr)>::private_class_if_failed {}
//--------------------------------------------------------------------------------
...
#pragma endregion //Many helper functions to help make the class work correctly
//--------------------------------------------------------------------------------
...
public:
	...
	int compare(const delegate_root& _other) const throw() {
		return strategy_root::compare(strategy(), _other.strategy());
	}
	bool operator==(const delegate_root& _other) const throw() {
		return strategy_root::compare(strategy(), _other.strategy()) == 0;
	}
	bool operator!=(const delegate_root& _other) const throw() {
		return strategy_root::compare(strategy(), _other.strategy()) != 0;
	}
	...
	struct strategy_root {
		...
		void init_null() throw() {
			memset(&m_object_ptr, 0, OFFSET_OF(strategy_root, m_fn) + sizeof(m_fn) - OFFSET_OF(strategy_root, m_object_ptr));
		}
		...
		static int compare(const strategy_root& _left, const strategy_root& _right) throw() {
			return memcmp(&(_left.m_object_ptr), &(_right.m_object_ptr), sizeof(_left.m_object_ptr) + sizeof(_left.m_fn));
		}
		void object_clone() throw() {
			if (0 != m_object_ptr) {
				on_clone_object(&reinterpret_cast<void*&>(m_object_ptr));
			}
		}
		void object_free() throw() {
			if (0 != m_object_ptr) {
				on_free_object(m_object_ptr);
			}
		}
	};
...
public:
	clone_option(bool clone_for_first_time = true) throw() : clone_for_first_time(clone_for_first_time) {
	}
	operator bool() const throw() {
		return clone_for_first_time;
	}
	...
	struct strategy : public delegate_root::strategy_root {
		...
		virtual void on_clone_object(void** pp) const throw() {
			base_type* cloned_ptr = clone_allocator_type::allocate_clone(static_cast<object_type&>(*static_cast<base_type*>(*pp)));
			*pp = cloned_ptr;
		}
		// free object
		virtual void on_free_object(void* p) const throw() {
			clone_allocator_type::deallocate_clone(static_cast<object_type*>(static_cast<base_type*>(p)));
		}
	};
};
//--------------------------------------------------------------------------------
...
struct clone_option<view_clone_allocator> {
	...
	struct no_clone_strategy : public delegate_root::strategy_root {
		virtual void on_clone_object(void** pp) const throw() {/*do nothing*/ }
		virtual void on_free_object(void* p) const throw() {/*do nothing*/ }
	};

	template<typename object_type, typename base_type> struct strategy : public no_clone_strategy {};
};
//--------------------------------------------------------------------------------